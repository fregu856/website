...
authors = ["**Fredrik K. Gustafsson**, TODO!, TODO!, TODO!, Mattias Rantalainen"]
...
#abstract_short = "Pathology foundation models (PFMs) have emerged as powerful pretrained encoders for computational pathology, but systematic comparisons for clinically meaningful prediction tasks remain limited. We benchmark 12 PFMs for breast cancer survival prediction from whole-slide histopathology images. We evaluate models across three independent datasets comprising more than 5,400 patients with long-term follow-up. H-optimus-1 achieves the best overall performance, and we observe consistent generational improvements across model families. However, absolute performance differences between many PFMs are small, suggesting diminishing returns from further scaling of pretraining data or model size alone. Notably, the compact distilled model H0-mini slightly outperforms its larger teacher model H-optimus-0, despite using fewer than 8% of the parameters. Together, these results provide the first large-scale, externally validated benchmark of PFMs for breast cancer survival prediction."
#abstract_short = "Pathology foundation models (PFMs) are central to computational pathology, yet systematic comparisons for clinically meaningful prediction tasks remain limited. We benchmark 12 PFMs for survival prediction from WSIs across three independent cohorts with over 5,400 breast cancer patients. H-optimus-1 achieves the best performance overall, with consistent generational improvements across model families. However, performance differences between recent PFMs are small, suggesting diminishing returns from further scaling of pretraining data or model size alone. Notably, the compact distilled model H0-mini slightly outperforms its larger teacher model, despite using fewer than 8% of the parameters. This provides the first large-scale, externally validated benchmark of PFMs for breast cancer survival prediction."
